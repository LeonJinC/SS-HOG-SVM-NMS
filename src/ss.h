...
namespace ss
{
	...
	//计算点(x1,y1)和点(x2,y2)在图像三个通道上的累加差方的根；也就是计算两个点在颜色空间的欧氏距离
	...
	{
		int rank;//顶点的等级，等级越高代表辈分越大
		int p;//该顶点的父节点编号
		int size;//该顶点的度，也就是和该顶点连接的节点数
	...
	public:
		...
		//输入x，x为图的某一个顶点，输出该顶点属于哪一个顶点的顶点编号
		int find(int x)
		{
			int y = x;
		...
		//输入x，y，如果x顶点的rank>y顶点的rank,则y顶点归属于x，x顶点的子树数量+1
		//如果x顶点的rank<=y顶点的rank,则x顶点归属于y，y顶点的子树数量+1,且如果x顶点的rank==y顶点的rank，则y顶点的rank+1
	...
	struct edge
	{
		int a;//起始点的y1*width+x1
		int b;//终点的y2*width+x2
		double w;//(x1,y1)和(x2,y2)在颜色空间上的欧氏距离
	};
	...
	{
		std::sort(edges.begin(), edges.end());//泛型排序算法函数sort需要重载“小于号运算符函数 operator< ”默认升序：也就是从小到大

		auto universe = std::make_shared<Universe>(numVertices);

		std::vector<double> threshold(numVertices, scale);//所有顶点的类内不相似度阈值
		...
		for (auto &pedge : edges)
		{
			...
			if (a != b)
			{
				...
				{//如果两个顶点的距离小于各自所属类别的不相似度阈值，也就是说两个类别足够的接近！
					universe->join(a, b);
					a = universe->find(a);
					threshold[a] = pedge.w + calThreshold(universe->size(a), scale);
					//更新该类的不相似度阈值，而且由于edges是根据w从小到大排序的，所以该类的不相似度阈值实则慢慢变大
					//scale能控制类的不相似度阈值，scale越大则类的不相似度阈值相对就越大，也就是说各个类别都更容易融合在一起。
				}
			}
		}
	...
	std::shared_ptr<Universe> segmentation(const cv::Mat &img, double scale, double sigma, int minSize)
	{
		...
		cv::cvtColor(imgF, imgF, CV_RGB2HSV);//H：色调，S：饱和度，V：明度//远比rgb效果更好
		//cv::cvtColor(imgF, imgF, CV_RGB2YCrCb);

		cv::Mat blurred;
		cv::GaussianBlur(imgF, blurred, cv::Size(5, 5), sigma);

		std::vector<edge> edges(width*height * 4);//图像的每个点计算4个颜色空间欧氏距离
		...
		for (int y = 0; y < height; y++)
		{
			for (int x = 0; x < width; x++)
			{
				if (x < width - 1)
				{
					edges[num].a = y * width + x;//起始顶点的编号
					edges[num].b = y * width + (x + 1);//终点顶点的编号
					edges[num].w = diff(blurred, x, y, x + 1, y);//两个顶点在颜色空间中的欧氏距离
					num++;
				}
		...
		for (int i = 0; i < num; i++)
		{
			...
			{//如果两个类别的size很小，小于minSize，则将两个类别合并
				universe->join(a, b);
			}
		}
	...
	//计算两个区域之间的大小相似度，
	//如果两个两个都很小则相似度很高，一大一小或者都很大则相似度低
	...
	//计算该label对应的区域包含的个体数量size，也就是该区域的面积大小
	...
	//将universe中记录的像素点及其信息（顶点等级rank，所属类别p，类别包含的个体数量size）进行整理整合，
	//得出真正有用的Region集合
	std::map<int, Region> extractRegions(const cv::Mat &img, std::shared_ptr<Universe> universe)
	{
		...
		for (int y = 0; y < img.rows; y++)
		{
			for (int x = 0; x < img.cols; x++)
			{
				...
				//为每一个region设置一个label，如果没有该label的region则创建！
				if (R.find(label) == R.end())
				{
					R[label] = Region(cv::Rect(100000, 100000, 0, 0), label);
				}

				//调整region的rect边框，该region对应的边框是恰好紧紧包围region的边框
				if (R[label].rect.x > x)//rect.x为边框左上角的x，左上角的x越来越小
				{
					R[label].rect.x = x;
				}

				if (R[label].rect.y > y)//rect.y为边框左上角的y，左上角的y越来越小
				{
					R[label].rect.y = y;
				}

				if (R[label].rect.br().x < x)//rect.br().x为边框右下角的x，右下角的x越来越大
				{
					R[label].rect.width = x - R[label].rect.x + 1;
				}

				if (R[label].rect.br().y < y)//rect.br().y为边框右下角的y，右下角的y越来越大
				{
		...
		cv::Mat gradient = calcTextureGradient(img);//数值在0~180之间
	...
	/*
	@param img:待检测的图像
	@param scale：图像分割算法中的贪心指数，越大则分割的区域面积更大类别更少，则用于选择搜索的区域更少
	@param sigma：高斯函数的sigma参数
	@param minSize：分割后会有很多小区域，当区域像素点的个数小于min时，选择与其差异最小的区域合并
	@param smallest：矩形最小面积
	@param largest：矩形最大面积
	@param distorted：h/w的最小比例阈值
	@return std::vector<cv::Rect> 预选边框
	*/
